...
class MalariaModel(pl.LightningModule):
    def __init__(self, num_classes=3):
        ...
        # Conexión residual para conv3
        self.identity_conv3 = nn.Conv2d(64, 128, kernel_size=1, stride=1, padding=0)
        ...
        self.dropout3 = nn.Dropout(0.1)  # Nuevo dropout para después de conv3
        
        # Calcula el tamaño de la salida de la última capa de pooling para dimensionar la capa FC
        self.fc1 = nn.Linear(self._calc_fc_input(), 256)  # Cambia las dimensiones según sea necesario
    ...
    def forward(self, x):
        ...
        x = self.dropout1(self.pool(self.leaky_relu(self.batchnorm1(self.conv1(x)))))

        # Segunda capa convolucional y pooling
        identity = x = self.dropout2(self.pool(self.leaky_relu(self.batchnorm2(self.conv2(x)))))

        # Preparar la identidad para la conexión residual
        # Aquí, identity_conv3 ajusta el número de canales de 64 a 128 para coincidir con la salida de conv3
        identity = self.identity_conv3(identity)

        # Tercera capa convolucional sin pooling aún
        x = self.leaky_relu(self.batchnorm3(self.conv3(x)))

        # Sumar la conexión residual
        x += identity

        # Pooling después de sumar la conexión residual
        x = self.pool(x)

        # Cuarta capa convolucional y pooling
        x = self.dropout3(self.pool(self.leaky_relu(self.batchnorm4(self.conv4(x)))))
    ...
    # Calcula el tamaño de las entradas planas para la primera capa FC
    # Esta es una función auxiliar para calcular automáticamente el tamaño
        with torch.no_grad():
            input_size = (1, 7, 100, 100)  # Cambia esto si el tamaño de entrada de la imagen es diferente
    ...
    def on_fit_end(self):
        ...
        plt.ylabel('Loss')
        plt.legend()
        plt.savefig('Tesis/figures/4/Graph.png')  
        plt.close()  
    ...
    def show_confusion_matrix(self, predictions, labels, name):
        ...
        plt.savefig(f'Tesis/figures/4/{name}_confusion_matrix.png')
        plt.close()
